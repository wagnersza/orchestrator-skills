...
| **simple-english** (skill) | **owns all writing rules for prose deliverables** — a worker runs the prose it changed through this skill before it commits, and the orchestrator applies the skill to its own reports. This repo restates no rule of the standard, only when to invoke it (see `../CONTEXT.md`) | `simple-english` check above | `npx skills add AminBlg/SimpleEnglish --global --all` — the [`skills` CLI](https://skills.sh), so it needs `node`/`npx` and network access. Repo <https://github.com/AminBlg/SimpleEnglish>, MIT, no dependencies of its own. Not a Claude plugin marketplace install, so it carries no pin (ADR 0010) |
