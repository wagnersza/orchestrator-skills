...
- **What the prohibition above forbids is one copy of one identical seam command in three tool files.** It does not reach a command that genuinely differs per tool. Operations 11 and 12 are that case: `orca` has an automation surface, and `cmux` and `herdr` have none. That difference is the shape a tool row exists for. So the two rows are permitted, and each tool file writes its own command around a `<precheck-command>` placeholder. The caller fills the placeholder with the seam command. So the seam stays in one place. Rationale: [`../../docs/adr/0022-item-automation-replaces-the-blocking-watch.md`](../../docs/adr/0022-item-automation-replaces-the-blocking-watch.md).
