<!--
Vendored from https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-sonnet-5
Fetched 2026-07-29. Refresh manually when the guide changes.
Used by: sonnet-5 (anthropic) and gpt-5.6 "terra" (openai) — terra follows this guide.
Read _composing.md (same folder) alongside this file — it holds the rules that
apply to every worker prompt regardless of model.
-->
...
On Claude Sonnet 5, adaptive thinking is on by default. Requests without a `thinking` field run with adaptive thinking. To turn thinking off entirely, pass `thinking: {type: "disabled"}`. Because `max_tokens` is a hard limit on total output (thinking plus response text), revisit it for workloads that ran without thinking on Claude Sonnet 4.6. If you were previously using thinking off with Claude Sonnet 4.6, try thinking on with lower effort levels for Claude Sonnet 5.
...
Manual extended thinking (`thinking: {type: "enabled", budget_tokens: N}`) is not supported on Claude Sonnet 5 and returns a 400 error. Use adaptive thinking with the effort parameter instead.
...
**1. Specify a concrete alternative.** The model follows explicit specs precisely — give exact hex palette, typeface and letter-spacing, corner radius, section structure, and transition timings. (The upstream guide carries a full worked example brief; the shape is "every visual decision named, no room left for a default".)
...
To steer away from generic patterns that users call the "AI slop" aesthetic, include a short directive in the system prompt:
...
Claude Sonnet 5 supports the `computer_20251124` tool version. Computer use works across resolutions, up to a maximum of 2576px / 3.75MP. Sending images at 1080p provides a good balance of performance and cost; for particularly cost-sensitive workloads, 720p or 1366×768 are lower-cost options with strong performance. Experimenting with effort settings can also help tune behavior.
