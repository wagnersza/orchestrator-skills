...
**Item automation**:
One schedule per live **Work item**, owned by the **Tool** rather than by a session's shell, named `orchestrator-item-<N>`. It ticks once a minute. Its precheck is the **Worker watch** seam asked as a predicate: exit 0 means a **Phase** transition is due, and any other code means nothing to do and the run records as skipped at no token cost. On exit 0 it wakes the **Orchestrator** with one line to the live orchestrator terminal. Where no such terminal resolves, it posts the same line as a comment on the work item.

**It never acts.** It writes no label, composes no prompt, spawns nothing and merges nothing. **The automation decides when, and the session decides what** — the same split as a **Close transaction** and a **Worker watch**, applied a third time. So every destructive act stays in a session a human can interrupt. One per item, so a leaked schedule names the item it leaked from, and five siblings are five observed items. Removal folds into step 8 of a **Close transaction**, which means a refused transaction leaves the item observed. A tool with no automation surface skips the tick and the spawn works unchanged. This entry names the operations before they exist: the term is declared here, and the tool rows land with the flow change that consumes them. Rationale, the schedule that replaces the blocking watch, the `dead` and `stalled` split, and the rejected alternatives: `docs/adr/0022-item-automation-replaces-the-blocking-watch.md`.
...
**The blocking poll loop is retiring, and the seam is not.** A loop in a background process of the orchestrator's own shell dies with that shell. It reports nothing when it does. So the trigger becomes an **Item automation**, and the seam is asked once per tick as a predicate. The exit-code contract survives, the statelessness survives, and the split above survives. What retires is `worker_state watch`, with its stall window and its bounded maximum wait. `docs/adr/0022-item-automation-replaces-the-blocking-watch.md` narrows ADR 0018 to that extent and no further.
