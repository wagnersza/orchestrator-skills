...
**Board status**:
Where a work item sits on the board, where the tracker has one. **A column is a card on one tracker and a scoped label on the other**: on GitHub it is the `Status` field on the item's card (GitHub Projects v2), and on GitLab it is a label under one scope, `status::to do` and its siblings. The adapter is where that difference lives, and no seam and no skill body learns which representation it holds. The GitLab read costs no call of its own, because every label already arrives with the item read ([`docs/adr/0070-a-board-column-is-a-card-or-a-scoped-label.md`](docs/adr/0070-a-board-column-is-a-card-or-a-scoped-label.md), which narrows the ADR below). A board built on plain column labels reads as unreadable there, because nothing swaps a plain label and two column labels can then sit on one item. **The board is a mirror of the work state, and a seam writes the card at three moments.** It is read in one column and written in three, and the two sets do not overlap. So `Status` is still no derived projection of the **Work-state labels**: there is no derivation table, no reconcile pass and no sync command. **The card arrives with the item, and a gate lists no board**: `Tracker.labelled_items` reads one labelled set and each item's card in one call, and `Tracker.item_card` reads the card of one item. `Tracker.board_cards` is the whole-board list, and the `report` verb is its one caller. `Tracker.card_write` is the one write. The two coordinates and every column name live in `docs/agents/issue-tracker.md`, alongside the labels. GitLab needs no project number and no owner there, because a column name is the whole coordinate where the label is the column. A GitHub repo with no board omits that section, the label alone is the whole gate, and no card write is attempted. **A GitLab repo has no such absence to declare**: the two coordinates are what a missing board looks like on GitHub, and GitLab reads neither. So there the three writes always run, and a project whose maintainer wants no board gains three labels nobody reads. Rationale: [`docs/adr/0067-the-board-is-a-mirror-at-three-moments.md`](docs/adr/0067-the-board-is-a-mirror-at-three-moments.md), which reverses [`docs/adr/0054-the-board-is-an-input-not-a-mirror.md`](docs/adr/0054-the-board-is-an-input-not-a-mirror.md), and [`docs/adr/0064-the-start-gate-reads-two-labelled-sets.md`](docs/adr/0064-the-start-gate-reads-two-labelled-sets.md).
...
failed column write is reported and stops nothing**, at all three moments and on both
trackers. The `Done` write is safe to repeat, so GitHub's own **item closed to Done** workflow
stays on beside it. The token needs the `project` scope.

**All three writes work on both trackers, and the adapter holds the whole difference.** On
GitHub each one resolves an option id from the column name. On GitLab each one is a single
label write, through the same label writer the **Work-state label** family uses. It adds the
scoped label for the target column and names none to remove, because GitLab drops the other
value of the same scope by itself. So a repeat write of the same column changes nothing there,
and it reads no board first. The two coordinates are not read on GitLab, because a column name
is the whole coordinate where the label is the column. **On GitLab this write is the only thing
that fills the done column**, because that tracker ships no **item closed to Done** workflow.
Both trackers end in the same column. The two families never meet. A work-state swap computes
its removals from the four work states, and a column write names no label to remove at all
([`docs/adr/0070-a-board-column-is-a-card-or-a-scoped-label.md`](docs/adr/0070-a-board-column-is-a-card-or-a-scoped-label.md)).
