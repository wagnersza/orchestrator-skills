...
**Skill reachability**:
Whether a routed skill can actually be entered, asked before every route and in both **Lane** values. Two halves, and both must hold. **The plugin that ships the skill is installed** — `references/requirements.md` holds that check. **And the model can invoke the skill** — a skill whose frontmatter holds `disable-model-invocation: true` is absent from the skill list of a session. So no model enters it, and only a human who types the slash command can. The second half is asked in one order. First, the name of the skill is in the skill list of this session. Second, where that list is not visible, the frontmatter carries no such flag. `references/skill-routing.md` holds the exact check.

**The two halves fail differently.** A missing plugin aborts a spawn, because nothing on disk holds the contract. A skill the model cannot invoke does not abort one. The plugin is installed and the skill body is there. So the **Worker** takes the row's *Without slash commands* prose, in the place the slash command holds. An **inline** route claims nothing instead. It asks the maintainer to type the command, or it answers the verb freehand and reports the skill as unreachable. Every spawn report names which branch it took.

**Nothing here names a skill.** The flag belongs to the upstream skill, and a plugin update moves it. So a list of blocked names reports a stale answer with no error. The routing table therefore gains no column and no per-skill flag, and no file in this repo records which skills carry the flag. Rationale, the rejected alternatives and the accepted risk: `docs/adr/0030-preflight-skill-reachability-before-routing.md`, which narrows `docs/adr/0014-route-verbs-to-skills-in-two-lanes.md` to that extent and no further.
_Avoid_: skill availability, installed (that names the first half alone), allowlist, blocklist, deny list (each names the hand-written list of names this deliberately is not), skill gate.
